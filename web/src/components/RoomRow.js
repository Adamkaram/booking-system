...
const rowMapper = (dayHours, props) => {
  ...
  for (var i = 0; i < 13; i++) {
    ...
    if (typeof bookingData == "number") {
      tableRow.push(
        <td key={`rowMapper${i}`} className="table__cell--available">
          <Link
            to="/createbooking"
            onClick={() => {
            ...
            <tr>
              <td className={`table__cell`}>
                <span
                  ...
                  className={`table__cell--booked table__cell--${firstBookingData.businessUnit
                    // Class name will show the business unit that made the booking, and whether the <td>
                    // element should be fully shaded, or half shaded (indicating a half-hour booking)
                    .replace(/ /g, "-")
                    .toLowerCase()}
                  `}>
                  &nbsp;
              ...
              <td className={`table__cell`}>
                <span
                  ...
                  className={`table__cell--booked table__cell--${secondBookingData.businessUnit
...
const RoomRow = (props) => (
  <tr className="table__row">
    <th scope="row" className="table__cell--align-left">
      ...
      <button style={{ Color: "whitesmoke" }}> <LockIcon fontSize="large" /></button>  
      &nbsp;
     <button style={{Color:"whitesmoke"}}> <LockOpenIcon fontSize="large"/></button>
    </th>
