...
const dateAEST = date => {
  return momentTimezone(date).tz('Africa/Cairo')
}
...
router.put('/rooms/:id', requireJWT, (req, res) => {
  ...
  // If the booking is a recurring booking
    ...
    let bookingDateTracker = momentTimezone(firstBooking.bookingStart).tz('Africa/Cairo')
    
    // A Moment.js date object for the final booking date in the recurring booking range - 
    //set to one hour ahead of the first booking - to calculate the number of days / weeks /
    //months between the first and last bookings when rounded down
    let lastBookingDate = momentTimezone(firstBooking.recurring[0]).tz('Africa/Cairo')
    ...
    for (let i = 0; i < bookingsInRange; i++) {
      ...
      if (proposedBookingDateStart.day() !== 0) {
        ...
        let firstBookingEndDate = momentTimezone(firstBooking.bookingEnd).tz('Africa/Cairo')
